...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($aid) {
        if ($action === 'valider') {
            ...
            flash('Association validée et activée.', 'success');
    ...
    $sid = $_POST['siege_id'] ?? '';
    if ($sid && $_POST['action'] === 'supprimer_siege') {
...
<div class="container py-20">
    <div class="filters-bar">
        <form method="GET" class="flex gap-10 items-center" style="flex-wrap:wrap;width:100%;">
            <div class="form-group mb-0"><label>Statut</label>
    ...
    <div class="table-wrapper">
        <table>
            <thead><tr><th>Nom</th><th>Président</th><th>Description</th><th>Sièges</th><th>Statut</th><th>Date création</th><th>Actions</th></tr></thead>
            <tbody>
            ...
            <tr>
                ...
                <td style="max-width:200px;"><?= htmlspecialchars(substr($a['description']??'—',0,80)) ?></td>
                <td class="text-center"><?= $a['nb_sieges'] ?></td>
                ...
                <td>
                    <form method="POST" class="flex gap-10" style="flex-wrap:wrap;">
                        ...
                        <button name="action" value="valider" type="submit" class="btn btn-success btn-sm" onclick="return confirm('Valider cette association ?')"><i class="fas fa-check"></i> Valider</button>
                        <button name="action" value="rejeter" type="submit" class="btn btn-warning btn-sm" onclick="return confirm('Rejeter cette association ?')"><i class="fas fa-times"></i> Rejeter</button>
                        <?php elseif ($a['statut'] === 'active'): ?>
                        <button name="action" value="rejeter" type="submit" class="btn btn-warning btn-sm" onclick="return confirm('Suspendre cette association ?')"><i class="fas fa-ban"></i></button>
                        <?php elseif (in_array($a['statut'],['rejetee','suspendue'])): ?>
                        <button name="action" value="valider" type="submit" class="btn btn-success btn-sm" onclick="return confirm('Réactiver cette association ?')"><i class="fas fa-check"></i></button>
                        <?php endif; ?>
                        <button name="action" value="supprimer" type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Supprimer définitivement cette association et tous ses sièges ?')"><i class="fas fa-trash"></i></button>
                    </form>
                </td>
            </tr>
            <?php endforeach; else: ?>
            <tr><td colspan="7" class="text-center py-20 text-light">Aucune association.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
